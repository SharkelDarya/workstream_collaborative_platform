...
class workspaceController {
    ...
    async getWorkspace(req, res) {
        try {
            ...
            res.json({ workspace, users, channels, jobs });
        } catch (error) {
            console.error(error);
    ...
    async createJob(req, res) {
        try {
            const { workspace_id, type } = req.body;
            const userId = req.user.id; 

            if (!workspace_id || !type) {
                return res.status(400).json({ message: "Missing workspace_id or type" });
            }
    ...
    async getJobs(req, res) {
        try {
            ...
            if (!workspace_id) {
                return res.status(400).json({ message: "Workspace not define!" });
            }

            const jobs = await Job.findAll({ where: { workspaceId: workspace_id } });

            if (!jobs) {
                return res.status(400).json({ message: "Jobs not define!" });
            }
    ...
    async deleteJob(req, res) {
        try {
            ...
            if (!workspace_id || !id_job) {
                return res.status(400).json({ message: "Workspace or job not define!" });
            }
            ...
            if (!job) {
                return res.status(404).json({ message: "Job not exist in workspace!" });
            }
